...
> ⚠️ **Disable PHPantom — it conflicts with rename.** Unlike hover and goto, which Zed merges across every attached server, a rename is answered by exactly **one** server, and Zed does not fall through to a second when the first declines. This extension claims `F2` on any class name resolving to a project file, so with [PHPantom](https://github.com/PHPantom-dev/phpantom_lsp) also attached its namespace renaming and move-on-rename are unreachable — it is never asked. Add `"!phpantom"` to your PHP `language_servers` list ([settings block](configuration.md#all-settings)). The one thing that costs you: our class rename is **same-namespace only**, so moving a class to another namespace stays a manual edit. Tracked in [#282](https://github.com/mike-bronner/zed-laravel/issues/282), with the longer-term division of labour in [#277](https://github.com/mike-bronner/zed-laravel/issues/277). See [🩺 Troubleshooting §5](troubleshooting.md#5-rename-does-nothing-on-a-class-name).
