...
//! `loadLivewireComponentsFrom(__DIR__.'/../Livewire', 'common-google')`,
//! no root `app/Livewire`) got zero `<livewire:` completions. The
//! conventional-path section is now skipped rather than short-circuiting
//! the whole function; the namespace walk always runs.
//!
...
async fn seed(
    ...
    *backend.cached_config.write().await = Some(Arc::new(config));
    backend
}
