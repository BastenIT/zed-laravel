...
pub fn build_watchers(
    ...
    // + 4 Inertia page-extension globs + 1 optional livewire + 2 per view
    // path + 1 per PSR-4 source root.
    let mut watchers = Vec::with_capacity(11 + 2 * view_paths.len() + psr4_roots.len());
